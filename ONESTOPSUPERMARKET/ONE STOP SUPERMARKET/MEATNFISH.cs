...
namespace ONE_STOP_SUPERMARKET
{
    ...
    public partial class MEAT_AND_FISH : Form
    {
        ...
        private List<MeatnFishClass> meatnfish = new List<MeatnFishClass>();
        private List<CartItem> cartItems = new List<CartItem>();
        private List<LikeItem> likeItems = new List<LikeItem>();
        DataTable meatfish = new DataTable();
        ...
        public MEAT_AND_FISH(Panel panel)
        {
            InitializeComponent();
            this.panel_main = panel;
        }

        private void MEAT_AND_FISH_Load(object sender, EventArgs e)
        {
            labelList.Clear();
            ...
            form2.sqlAdapter.Fill(meatfish);
            ...
            labelharga = new Label[meatfish.Rows.Count];
            ...
            for (int i = 47; i <= 51; i++)
            {
                ...
                picturebox[i].Location = new Point(50, (i - 47) * 300 + 80);
                this.Controls.Add(picturebox[i]);

                Label idproduk = new Label();
                Label labelmeatnfish = new Label();
                labelmeatnfish.Text = meatfish.Rows[i]["namaproduk"].ToString();
                labelmeatnfish.Tag = meatfish.Rows[i]["idproduk"].ToString();
                labelmeatnfish.AutoSize = true;
                labelmeatnfish.Font = new Font("Ebrima", 15, FontStyle.Bold);
                labelmeatnfish.Location = new Point(325, (i - 47) * 300 + 80);
                this.Controls.Add(labelmeatnfish);

                Label labelrp = new Label();
                labelrp.Text = "Rp.";
                labelrp.AutoSize = true;
                labelrp.Font = new Font("Ebrima", 12, FontStyle.Bold);
                labelrp.Location = new Point(325, (i - 47) * 300 + 120);
                this.Controls.Add(labelrp);

                labelharga[i] = new Label();
                int harga = Convert.ToInt32(meatfish.Rows[i]["hargacampur"].ToString());
                labelharga[i].Text = harga.ToString();
                labelharga[i].AutoSize = true;
                labelharga[i].Font = new Font("Ebrima", 12, FontStyle.Bold);
                labelharga[i].Location = new Point(355, (i - 47) * 300 + 120);
                this.Controls.Add(labelharga[i]);

                for (int y = 0; y < meatfish.Rows.Count; y++)
                {
                    int promosi = Convert.ToInt32(meatfish.Rows[y]["hargapromo"]);

                    if (promosi != 0)
                    {
                        if (labelmeatnfish.Text == meatfish.Rows[y]["namaproduk"].ToString())
                        {
                            labelharga[i].ForeColor = Color.Red;
                            Label labelhargapromo = new Label();
                            int hargapromo = Convert.ToInt32(meatfish.Rows[y]["hargaproduk"]);
                            ...
                            labelhargapromo.Location = new Point(420, (i - 47) * 300 + 120);
                ...
                buttonmin.Tag = i;
                buttonmin.Location = new Point(325, (i - 47) * 300 + 155);
                ...
                labelnol.Location = new Point(360, (i - 47) * 300 + 160);
                ...
                buttonbuy.Location = new Point(325, (i - 47) * 300 + 290);
                buttonbuy.Tag = "" + labelmeatnfish.Text.ToString();
                ...
                buttonfav.Location = new Point(450, (i - 47) * 300 + 290);
                buttonfav.Tag = i-47;
                buttonfav.Click += Buttonfav_Click;
                this.Controls.Add(buttonfav);

                Label labeldeskripsi = new Label();
                labeldeskripsi.Text = meatfish.Rows[i]["deskripsi"].ToString();
                labeldeskripsi.AutoSize = true;
                labeldeskripsi.Font = new Font("Serif", 10, FontStyle.Regular);
                labeldeskripsi.Location = new Point(325, (i-47) * 300 + 200);
                labeldeskripsi.MaximumSize = new Size(350, 200);
                this.Controls.Add(labeldeskripsi);

                meatnfish.Add(new MeatnFishClass()
                {
                    MeatnFishImage = $@".\FOTO PRODUK\{i}.jpg",
                    MeatnFishName = labelmeatnfish.Text,
                    MeatnFishPrice = Convert.ToDecimal(labelharga[i].Text),
                    MeatnFishQuantity = 0,
                    MeatnFishTotal = 0,
                    MeatnFishID = meatfish.Rows[i]["idproduk"].ToString()
                });
            }
        }
        ...
        private void Buttonfav_Click(object sender, EventArgs e)
        {
            if (Login.isLoggedIn)
            {
                ...
                if (index >= 0 && index < meatnfish.Count)
                {
                    MeatnFishClass selectedmeatnfish = meatnfish[index];
                    bool isAlreadyLiked = false;

                    foreach (LikeItem likedItem in likeItems)
                    {
                        if (likedItem.Name == selectedmeatnfish.MeatnFishName)
                        {
                            isAlreadyLiked = true;
                            break;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            ID = selectedmeatnfish.MeatnFishID,
                            Name = selectedmeatnfish.MeatnFishName,
                            ImagePath = project + selectedmeatnfish.MeatnFishImage,
                            Price = selectedmeatnfish.MeatnFishPrice
                        };
                        try
                        {
        ...
        private void Buttonbuy_Click(object sender, EventArgs e)
        {
            if (Login.isLoggedIn)
            {
                ...
                if (qty > 0)
                {
                    MeatnFishClass selectedmeatnfish = null;

                    foreach (MeatnFishClass meatnfish in meatnfish)
                    {
                        if (meatnfish.MeatnFishName == tags)
                        {
                            selectedmeatnfish = meatnfish;
                            break;
                        }
                    }

                    if (selectedmeatnfish != null)
                    {
                        CartItem cartItem = new CartItem()
                        {
                            ID = selectedmeatnfish.MeatnFishID,
                            ImagePath = selectedmeatnfish.MeatnFishImage,
                            Name = selectedmeatnfish.MeatnFishName,
                            Price = selectedmeatnfish.MeatnFishPrice,
                            Quantity = qty
                        };
            ...
            {
                MessageBox.Show("You have to log in first.", "Sory!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void Buttonplus_Click(object sender, EventArgs e)
        {
            ...
            Label label = labelList[index - 47];
            int value = Convert.ToInt32(labelList[index - 47].Text);
            ...
            else
            {
                ...
                if (value <= 100)
                {
                    ...
                    meatnfish[index - 47].tambahQuantity();
                }
                else
                {
                    label.Text = "100";
            ...
            int harga = Convert.ToInt32(meatfish.Rows[index]["hargacampur"]);
        ...
        internal class MeatnFishClass
        {
            public string MeatnFishName { get; set; }
            public decimal MeatnFishPrice { get; set; }
            public int MeatnFishQuantity { get; set; }
            public decimal MeatnFishTotal { get; set; }
            public string MeatnFishImage { get; set; }
            public string MeatnFishID { get; set; }
